...
`
	defaultRedshiftRotateRootCredentialsSQL = `
...
func new() *Redshift {
	...
	connProducer.Type = redshiftTypeName
...
func (p *Redshift) RenewUser(ctx context.Context, statements dbplugin.Statements, username string, expiration time.Time) error {
	...
	if len(renewStmts) == 0 {
		renewStmts = []string{defaultRedshiftRenewSQL}
	}
...
func (p *Redshift) defaultRevokeUser(ctx context.Context, username string) error {
	...
	var exists bool
	err = db.QueryRowContext(ctx, "SELECT exists (SELECT username FROM pg_user WHERE username=$1);", username).Scan(&exists)
...
func (p *Redshift) RotateRootCredentials(ctx context.Context, statements []string) (map[string]interface{}, error) {
	...
	if len(rotateStatents) == 0 {
		rotateStatents = []string{defaultRedshiftRotateRootCredentialsSQL}
	}
